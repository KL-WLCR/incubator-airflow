...
def connections(args):
    ...
    def is_connection_equals(first, second):
        if (first.conn_id == second.conn_id and
            ...
            first.port == second.port and
            first.get_extra() == second.get_extra()):
            return True
        else:
            return False
    if args.list:
    ...
    if args.upsert:
        ...
        if not (conn):
            ...
            conn.conn_id = new_conn.conn_id
            conn.conn_type = new_conn.conn_type
            conn.host = new_conn.host
            conn.login = new_conn.login
            conn.password = new_conn.password
            conn.schema = new_conn.schema
            conn.port = new_conn.port
            conn._extra = new_conn._extra
            session.commit()
